...
export async function withdrawalRequest(req: Request, res: Response) {
  ...
  const { amount, paymentMethod, accountNumber, accountTitle, bankName, iban } =
    value;
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: "Failed to submit withdrawal request." });
  }
}
